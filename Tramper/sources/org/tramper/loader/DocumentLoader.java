...
public class DocumentLoader implements Loader, Runnable {
    ...
    protected void download() {
        ...
        try {
            ...
            try {
                ...
                try {
                    InputStream otherStream = browser.openRead(urlObj);
                    aParser = ParserFactory.getParserByDocumentRoot(otherStream);
                } catch (Exception re) {
                    logger.error("unknown document format", re);
                    LoadingEvent failedEvent = new LoadingEvent(this, null, loadingType, url, target);
                    fireLoadingFailedEvent(failedEvent);
                    return;
                }
    ...
    protected void upload() {
        ...
        aParser.setUrl(urlObj);
        try {
            aParser.unparse(outStream, uploadDocument);
    ...
    protected void call() {
        ...
        try {
            ...
            try {
                ...
                try {
                    InputStream otherStream = browser.openRead(urlObj, parameter);
                    aParser = ParserFactory.getParserByDocumentRoot(otherStream);
                } catch (Exception re) {
                    logger.error("unknown document format", re);
                    LoadingEvent failedEvent = new LoadingEvent(this, null, loadingType, url, target);
