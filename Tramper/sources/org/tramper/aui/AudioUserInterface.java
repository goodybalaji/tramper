...
public class AudioUserInterface implements UserInterface, LibraryListener {
	...
	Player docPlayer = null;
        ...
        try {
            docPlayer = PlayerFactory.getPlayerByMimeType(mimeType);
            docPlayer.render(document, target);
        } catch (Exception e) {
            String extension = document.getExtension();
            try {
        	docPlayer = PlayerFactory.getPlayerByExtension(extension);
                docPlayer.render(document, target);
            } catch (Exception e1) {
                logger.error("unable to get a player for the document");
                return;
            }
        }
